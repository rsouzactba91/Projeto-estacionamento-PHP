...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($acao) {
        case "inserir":
            $sql = "INSERT INTO tabeladevalores (idfaixa, valor) VALUES ('$idfaixa', '$valor')";
            $mensagem_acao = "Inserção";
            break;

        case "alterar":
            $sql = "UPDATE tabeladevalores SET valor='$valor' WHERE idfaixa='$idfaixa'";
            $mensagem_acao = "Atualização";
            break;

        case "deletar":
            $sql = "DELETE FROM tabeladevalores WHERE idfaixa='$idfaixa'";
    ...
    if (isset($sql) && !empty($sql)) {
        ...
        if ($mysqli->query($sql) === TRUE) {
            header('Location: definirtabela.php'); // Redireciona para 'definirtabela.php' após a execução bem-sucedida.
            exit;
        } else {
